...
# =========================================================
# LANDING PAGE — public marketing page at "/" for logged-out web visitors.
# Never shown inside the native app shell (is_native_app()) — that goes
# straight to /login instead, so the Capacitor/TWA build never renders
# pricing here either. See dashboard() below for the gate.
# =========================================================
def landing_page():
    demo_text = "Pr 2476 bayview ave,vb, froehler 30yd dump dominion note before you return it take to 1013 paragon way to swap out"

    plan_cards = ""
    for key, label, price, desc, cta_label, cta_href in [
        ("trial",   "Free",   "$0",     "14 days &middot; up to 5 drivers",              "Start Free Trial", url_for("company_register")),
        ("starter", "Starter","$49/mo", "Up to 10 drivers",                               "Start Free Trial", url_for("company_register")),
        ("pro",     "Pro",    "$99/mo", "Up to 30 drivers &middot; priority support",     "Start Free Trial", url_for("company_register")),
        ("fleet",   "Fleet",  "Custom", "Unlimited drivers &middot; dedicated support",   "Contact Us",       "mailto:info@haultraai.com"),
    ]:
        featured = ' lp-plan--featured' if key == "pro" else ''
        plan_cards += f"""
        <div class="lp-plan{featured}">
            {'<div class="lp-plan-badge">MOST POPULAR</div>' if key == "pro" else ''}
            <div class="lp-plan-name">{label}</div>
            <div class="lp-plan-price">{price}</div>
            <div class="lp-plan-desc">{desc}</div>
            <a class="lp-btn lp-btn--outline" href="{cta_href}">{cta_label}</a>
        </div>"""
...
<head>
<meta charset="utf-8">
<title>HAULTRA — Dispatch built for roll-off haulers</title>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<meta name="theme-color" content="#FF6B1A">
<meta name="description" content="HAULTRA turns a messy dispatch text into a structured, trackable route in seconds. Route dispatch, container tracking, and photo proof for roll-off haulers.">
...
<style>
* {{ box-sizing: border-box; }}
html, body {{ margin: 0; padding: 0; background: var(--bg-0); color: var(--text); font-family: var(--font-body); scroll-behavior: smooth; }}
body {{ overflow-x: hidden; }}
a {{ color: inherit; }}
.lp-wrap {{ max-width: 1080px; margin: 0 auto; padding: 0 20px; }}

/* ── Nav ── */
.lp-nav {{ position: sticky; top: 0; z-index: 20; background: rgba(18,18,18,0.86); backdrop-filter: blur(8px); border-bottom: 1px solid rgba(255,255,255,0.06); }}
.lp-nav-inner {{ max-width: 1080px; margin: 0 auto; padding: 14px 20px; display: flex; align-items: center; justify-content: space-between; }}
.lp-logo {{ font-family: var(--font-head); font-size: 22px; letter-spacing: 1px; }}
.lp-logo span {{ background: linear-gradient(120deg, #ffffff 0%, var(--text) 55%, var(--orange) 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }}
.lp-nav-login {{ font-size: 14px; font-weight: 600; color: var(--text-dim); text-decoration: none; padding: 8px 14px; border-radius: var(--radius-sm); border: 1px solid rgba(255,255,255,0.12); transition: color .15s ease, border-color .15s ease; }}
.lp-nav-login:hover {{ color: var(--text); border-color: rgba(255,255,255,0.3); }}

/* ── Buttons ── */
.lp-btn {{ display: inline-block; font-family: var(--font-body); font-weight: 700; font-size: 15px; padding: 13px 28px; border-radius: var(--radius-sm); text-decoration: none; transition: transform .15s ease, box-shadow .15s ease; }}
.lp-btn--primary {{ background: var(--orange); color: #1A0D00; box-shadow: 0 0 0 rgba(255,107,26,0); }}
.lp-btn--primary:hover {{ transform: translateY(-2px); box-shadow: 0 8px 24px rgba(255,107,26,0.35); }}
.lp-btn--outline {{ background: transparent; color: var(--text); border: 1px solid rgba(255,255,255,0.18); width: 100%; text-align: center; padding: 11px 20px; }}
.lp-btn--outline:hover {{ border-color: var(--orange); color: var(--orange); }}
.lp-secondary-link {{ color: var(--text-dim); font-weight: 600; font-size: 15px; text-decoration: none; margin-left: 18px; }}
.lp-secondary-link:hover {{ color: var(--text); }}

/* ── Hero ── */
.lp-hero {{ padding: 64px 0 20px; text-align: center; }}
.lp-hero h1 {{ font-family: var(--font-head); font-size: clamp(34px, 7vw, 58px); line-height: 1.05; letter-spacing: .5px; margin: 0 0 18px; text-wrap: balance; }}
...
/* ── Problem section ── */
.lp-problem {{ padding: 20px 0 70px; text-align: center; }}
.lp-problem h2 {{ font-family: var(--font-head); font-size: clamp(26px, 5vw, 36px); letter-spacing: .5px; margin: 0 0 10px; }}
.lp-problem > .lp-wrap > p {{ color: var(--text-dim); max-width: 520px; margin: 0 auto 34px; }}
.lp-problem-grid {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(190px, 1fr)); gap: 16px; }}
.lp-problem-card {{ background: var(--bg-1); border: 1px solid rgba(255,82,82,0.18); border-radius: var(--radius); padding: 22px 18px; text-align: left; }}
.lp-problem-card .glyph {{ font-size: 22px; margin-bottom: 10px; }}
.lp-problem-card h3 {{ font-size: 15px; margin: 0 0 6px; }}
.lp-problem-card p {{ color: var(--text-dim); font-size: 13.5px; line-height: 1.55; margin: 0; }}

/* ── Features ── */
.lp-features {{ padding: 30px 0 70px; }}
.lp-feature {{ display: grid; grid-template-columns: 1fr; gap: 22px; align-items: center; padding: 38px 0; border-top: 1px solid rgba(255,255,255,0.06); }}
.lp-feature:first-of-type {{ border-top: none; }}
@media (min-width: 760px) {{
  .lp-feature {{ grid-template-columns: 1fr 1fr; gap: 48px; }}
  .lp-feature.reverse .lp-feature-visual {{ order: -1; }}
}}
.lp-feature-eyebrow {{ font-size: 12px; letter-spacing: 1px; text-transform: uppercase; color: var(--orange); font-weight: 700; margin-bottom: 8px; }}
.lp-feature-eyebrow.is-ai {{ color: var(--teal); }}
.lp-feature h3 {{ font-family: var(--font-head); font-size: 26px; letter-spacing: .5px; margin: 0 0 10px; }}
.lp-feature p {{ color: var(--text-dim); font-size: 14.5px; line-height: 1.65; margin: 0; }}
.lp-feature-visual {{ background: var(--bg-1); border: 1px solid rgba(255,255,255,0.08); border-radius: var(--radius); padding: 22px; min-height: 160px; }}

/* Route board lanes mock */
.lp-lanes {{ display: flex; gap: 10px; }}
.lp-lane {{ flex: 1; background: var(--bg-2); border-radius: var(--radius-sm); padding: 10px; }}
.lp-lane-head {{ font-size: 11px; color: var(--text-faint); margin-bottom: 8px; }}
.lp-lane-card {{ background: var(--bg-0); border-radius: 7px; padding: 7px 9px; font-size: 11.5px; margin-bottom: 6px; border-left: 3px solid var(--orange); }}
.lp-lane-card.slate {{ border-left-color: var(--slate); }}
.lp-lane-card.done {{ border-left-color: var(--green); opacity: .6; }}

/* Bin tracker mock */
.lp-bins {{ position: relative; height: 160px; }}
.lp-bin-pin {{ position: absolute; width: 30px; height: 30px; border-radius: 50% 50% 50% 0; background: var(--orange); transform: rotate(-45deg) scale(0); opacity: 0; }}
.lp-bin-pin.overdue {{ background: var(--red); }}
.reveal.is-visible .lp-bin-pin {{ animation: lp-pin-drop .5s cubic-bezier(.34,1.56,.64,1) forwards; }}
.reveal.is-visible .lp-bin-pin:nth-child(1) {{ animation-delay: .05s; }}
.reveal.is-visible .lp-bin-pin:nth-child(2) {{ animation-delay: .22s; }}
.reveal.is-visible .lp-bin-pin:nth-child(3) {{ animation-delay: .39s; }}
.reveal.is-visible .lp-bin-pin:nth-child(4) {{ animation-delay: .56s; }}
@keyframes lp-pin-drop {{ 0% {{ opacity: 0; transform: rotate(-45deg) scale(0) translateY(-14px); }} 60% {{ opacity: 1; }} 100% {{ opacity: 1; transform: rotate(-45deg) scale(1) translateY(0); }} }}
.lp-clock-demo {{ margin-top: 14px; text-align: center; }}
.lp-clock-badge {{ display: inline-block; font-family: var(--font-mono); font-weight: 700; font-size: 13px; padding: 6px 14px; border-radius: 999px; background: var(--orange-dim); border: 1px solid var(--orange-border); color: var(--orange); transition: background .2s ease, border-color .2s ease, color .2s ease; }}
.lp-clock-badge.is-overdue {{ background: var(--red-dim); border-color: rgba(255,82,82,0.5); color: var(--red); }}

/* Photo proof mock */
.lp-proof {{ display: flex; gap: 10px; align-items: center; }}
.lp-proof-thumb {{ flex: 1; aspect-ratio: 1; border-radius: 9px; background: linear-gradient(135deg, var(--bg-2), var(--bg-0)); border: 1px dashed rgba(255,255,255,0.15); display: flex; align-items: center; justify-content: center; font-size: 20px; }}
.lp-gps-chip {{ font-family: var(--font-mono); font-size: 11px; color: var(--green); background: var(--green-dim); border: 1px solid rgba(61,220,132,0.35); padding: 4px 9px; border-radius: 999px; margin-top: 8px; display: inline-block; }}

/* ── Pricing ── */
.lp-pricing {{ padding: 30px 0 80px; text-align: center; }}
.lp-pricing h2 {{ font-family: var(--font-head); font-size: clamp(26px, 5vw, 36px); letter-spacing: .5px; margin: 0 0 10px; }}
.lp-pricing > .lp-wrap > p {{ color: var(--text-dim); margin: 0 0 34px; }}
.lp-plans-grid {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px; text-align: left; }}
.lp-plan {{ position: relative; background: var(--bg-1); border: 1px solid rgba(255,255,255,0.08); border-radius: var(--radius); padding: 26px 20px; display: flex; flex-direction: column; }}
.lp-plan--featured {{ border-color: var(--orange-border); background: linear-gradient(180deg, rgba(255,107,26,0.06), var(--bg-1) 60%); }}
.lp-plan-badge {{ position: absolute; top: -11px; left: 50%; transform: translateX(-50%); background: var(--orange); color: #1A0D00; font-size: 10px; font-weight: 800; letter-spacing: .5px; padding: 3px 10px; border-radius: 999px; }}
.lp-plan-name {{ font-weight: 700; font-size: 15px; margin-bottom: 6px; }}
.lp-plan-price {{ font-family: var(--font-head); font-size: 30px; letter-spacing: .5px; margin-bottom: 6px; }}
.lp-plan-desc {{ color: var(--text-dim); font-size: 13px; margin-bottom: 20px; flex: 1; }}

/* ── Footer ── */
.lp-footer {{ border-top: 1px solid rgba(255,255,255,0.06); padding: 34px 0 40px; text-align: center; }}
.lp-footer a {{ color: var(--text-dim); text-decoration: none; font-size: 13.5px; }}
.lp-footer a:hover {{ color: var(--text); }}
.lp-footer .sep {{ color: var(--text-faint); margin: 0 10px; }}
.lp-footer .copy {{ color: var(--text-faint); font-size: 12px; margin-top: 14px; }}
...
<nav class="lp-nav">
    <div class="lp-nav-inner">
        <span class="lp-logo">HAULTRA<span>.</span></span>
        <a class="lp-nav-login" href="{url_for('login')}">Log In</a>
    </div>
</nav>
...
<section class="lp-problem">
    <div class="lp-wrap">
        <h2>Dispatching by text doesn't scale</h2>
        <p>Every extra text thread is a stop that can get lost.</p>
        <div class="lp-problem-grid reveal">
            <div class="lp-problem-card">
                <div class="glyph">&#128683;</div>
                <h3>Stops fall through</h3>
                <p>Buried in a text thread, a stop gets missed and nobody notices until the customer calls.</p>
            </div>
            <div class="lp-problem-card">
                <div class="glyph">&#128248;</div>
                <h3>No proof of work</h3>
                <p>Disputes over what got picked up, dropped, or swapped — with nothing to point to.</p>
            </div>
            <div class="lp-problem-card">
                <div class="glyph">&#8987;</div>
                <h3>No container tracking</h3>
                <p>Bins sit on-site for weeks with no one tracking rental days until it's already overdue.</p>
            </div>
        </div>
    </div>
</section>

<section class="lp-features lp-wrap">
    <div class="lp-feature reveal">
        <div>
            <div class="lp-feature-eyebrow is-ai">AI Route Building</div>
            <h3>Text in. Structured route out.</h3>
            <p>Paste the same messy dispatch text you'd normally send a driver. HAULTRA parses
            addresses, action codes, container sizes, and dump locations into clean, assignable
            stops — teal-tagged wherever the AI did the work.</p>
        </div>
        <div class="lp-feature-visual">
            <div class="lp-demo-card" style="border-color:var(--teal-border);">
                <div class="lp-demo-card-top">
                    <span class="action-badge pickup">PR</span>
                    <span class="lp-demo-addr">2476 Bayview Ave</span>
                    <span class="lp-demo-ai-tag">AI&#8209;PARSED</span>
                </div>
                <div class="lp-demo-meta">30&nbsp;YD &middot; Dump: Dominion</div>
            </div>
        </div>
    </div>

    <div class="lp-feature reverse reveal">
        <div>
            <div class="lp-feature-eyebrow">Route Board</div>
            <h3>Every driver's day, one board.</h3>
            <p>Stops laid out in lanes by driver, live status at a glance. No more asking
            "who's got the Bayview pickup" over text.</p>
        </div>
        <div class="lp-feature-visual">
            <div class="lp-lanes">
                <div class="lp-lane">
                    <div class="lp-lane-head">Mike</div>
                    <div class="lp-lane-card">Bayview Ave</div>
                    <div class="lp-lane-card done">Sykes Ave</div>
                </div>
                <div class="lp-lane">
                    <div class="lp-lane-head">Dana</div>
                    <div class="lp-lane-card slate">Paragon Way</div>
                    <div class="lp-lane-card">Holland Rd</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="lp-feature reveal">
        <div>
            <div class="lp-feature-eyebrow">Bin Tracker</div>
            <h3>Rental clocks that catch overdue bins.</h3>
            <p>Every container out gets a rental clock, mapped and tracked automatically from
            GPS-stamped stop completions — so nothing sits forgotten on a job site.</p>
        </div>
        <div class="lp-feature-visual">
            <div class="lp-bins">
                <div class="lp-bin-pin" style="top:20px; left:15%;"></div>
                <div class="lp-bin-pin" style="top:60px; left:42%;"></div>
                <div class="lp-bin-pin overdue" style="top:15px; left:68%;"></div>
                <div class="lp-bin-pin" style="top:75px; left:85%;"></div>
            </div>
            <div class="lp-clock-demo">
                <span class="lp-clock-badge" id="lp-clock-badge">Day 1</span>
            </div>
        </div>
    </div>
    ...
    <div class="lp-feature reverse reveal">
        <div>
            <div class="lp-feature-eyebrow">Photo Proof + GPS</div>
            <h3>Proof, stamped automatically.</h3>
            <p>Drivers snap a photo at completion — GPS coordinates attach automatically,
            no manual entry, no disputes.</p>
        </div>
        <div class="lp-feature-visual">
            <div class="lp-proof">
                <div class="lp-proof-thumb">&#128247;</div>
                <div class="lp-proof-thumb">&#128247;</div>
            </div>
            <span class="lp-gps-chip">&#128205; GPS captured at completion</span>
        </div>
    </div>
</section>
...
<section class="lp-pricing">
    <div class="lp-wrap">
        <h2>Simple, per-driver pricing</h2>
        <p>Start free. Upgrade when your fleet grows.</p>
        <div class="lp-plans-grid reveal">
            {plan_cards}
        </div>
    </div>
</section>

<footer class="lp-footer lp-wrap">
    <div>
        <a href="{url_for('privacy_policy')}">Privacy Policy</a><span class="sep">&middot;</span>
        <a href="{url_for('terms_of_service')}">Terms of Service</a><span class="sep">&middot;</span>
        <a href="{url_for('support_page')}">Support</a><span class="sep">&middot;</span>
        <a href="mailto:info@haultraai.com">info@haultraai.com</a>
    </div>
    <div class="copy">&copy; {datetime.now().year} HAULTRA AI SYSTEMS &mdash; Built for the hauling industry.</div>
</footer>
...
(function() {{
    ...
    var clockBadge = document.getElementById('lp-clock-badge');
    if (clockBadge) {{
        if (reduceMotion) {{
            clockBadge.textContent = 'OVERDUE';
            clockBadge.classList.add('is-overdue');
        }} else {{
            var started = false;
            var startClock = function() {{
                ...
                var tick = setInterval(function() {{
                    if (day <= target) {{
                        clockBadge.textContent = 'Day ' + day;
                        day++;
                    }} else {{
                        clockBadge.textContent = 'OVERDUE';
                        clockBadge.classList.add('is-overdue');
            ...
            if ('IntersectionObserver' in window) {{
                var clockIo = new IntersectionObserver(function(entries) {{
                    entries.forEach(function(entry) {{
                        if (entry.isIntersecting) {{ startClock(); clockIo.disconnect(); }}
                    }});
                }}, {{ threshold: 0.4 }});
                clockIo.observe(clockBadge);
            }} else {{
                startClock();
            }}
        }}
...
def support_page():
    ...
    <div class="hero">
        <h1>Support</h1>
        <p class="muted small">We're a small team — real humans read every message.</p>
    </div>

    <div class="card" style="max-width:640px;line-height:1.8;">
        <p>Questions about your account, billing, a bug you've hit, or anything else —
        email us and we'll get back to you, typically within one business day.</p>
        <p style="margin-top:18px;">
            <a href="mailto:info@haultraai.com" class="btn" style="display:inline-block;">
                &#9993; info@haultraai.com
            </a>
        </p>
        <p class="muted small" style="margin-top:22px;">
            Looking for something specific? <a href="{url_for('privacy_policy')}">Privacy Policy</a>
            &middot; <a href="{url_for('terms_of_service')}">Terms of Service</a>
            &middot; <a href="{url_for('delete_account_request')}">Delete Your Account</a>
        </p>
    </div>
    """
    return render_template_string(shell_page("Support", body))
